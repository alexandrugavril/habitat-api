...
from habitat import VectorEnvSharedMem
...
def construct_envs_shared_mem(
    ...
    for i in range(num_processes):
        ...
        config.defrost()
        config.TASK_CONFIG = task_config
        config.SHARED_DATA = shared_tensors
        config.freeze()
        configs.append(config.clone())

    envs = habitat.VectorEnvSharedMem(
        make_env_fn=make_env_fn,
        env_fn_args=tuple(
            tuple(zip(configs, env_classes, range(num_processes)))
        ),
        shared_mem=shared_tensors
    )
    return envs
